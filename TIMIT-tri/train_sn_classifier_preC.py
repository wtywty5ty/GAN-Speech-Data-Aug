...
class CDCGAN_Classifier(object):
    def __init__(self, generator, discriminator, opt, device, phoneMap):
        self.opt = opt
        self.device = device
        # data
        ...
        self.C = torch.load(opt.C).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--C', default='outf/sn_classifier_critic2/log_aa_80ep/netC_.pkl', help='net classifier')
    parser.add_argument('--outf', default='outf/sn_classifier_critic1/log_aa_preC_m2', help="path to output files)")
    ...
    CDCGAN_Classifier(_netG, _netD, opt, device, phoneMap).train()
